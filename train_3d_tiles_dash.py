...
def get_unet(tile_size, num_channels, activation=lrelu):  # activation='relu'   activation=lrelu
    '''
    Create a 3D Unet with the specified size, number of filters and compile it
    with the given loss function and metrics.
    tile_size: Size of the image tile that represents the size of the input to the Unet
    num_channels: Number of channels
    activation: Defaults to custom leaky relu
    return: Compiled Keras Unet model
    '''
    ...
    model.compile(
        ...
        loss='mse',  # loss='binary_crossentropy', loss=custom_objects.weighted_tf_bc(), loss='mse', loss=custom_objects.mcc_loss
        metrics=['mse', 'binary_crossentropy', custom_objects.weighted_tf_bc(), custom_objects.matthews_cc]
        )  #  epsilon=0.1,
...
def train(
            project_name, train_folder, validation_folder,
            channel_zero_index=0, image_extension='.tif',
            tile_size=(128,128,8), overlap_fraction=(0.5,0.5,0.5),
            force_z=8, batch_size=32, epochs=300, 
        ):
    '''
    channel_zero_index: Index (zero indexed) of single channel to process
    force_z: Force the z-dimension to a specific number of slices
    tile_size: Tuple of x,y,z tile dimensions
    overlap_fraction: Tuple of x,y,z tile overlap frations between 0 and 1
    image_extension: Extension of image file (ex. .tif)
    ...
    #         loss=custom_objects.mcc_loss,  # loss='binary_crossentropy', loss=custom_objects.weighted_tf_bc(), loss='mse', loss=custom_objects.mcc_loss
    #         metrics=['mse', 'binary_crossentropy', custom_objects.weighted_tf_bc(), custom_objects.matthews_cc]
    #         )  #  epsilon=0.1,
    ...
    callbacks = [
        ...
        custom_objects.ValImageCallback(log_dir, validation_folder, image_extension, tile_size, overlap_fraction, save_on_epoch=20, channel_zero_index=channel_zero_index, force_z=force_z),
    ]
    model.fit(
